...
public class RequestUtil {
    ...
    /**
     * 获取用户真实IP地址，不直接使用request.getRemoteAddr();的原因是有可能用户使用了代理软件方式避免真实IP地址, <br>
     *
     * 可是，如果通过了多级反向代理的话，X-Forwarded-For的值并不止一个，而是一串IP值，究竟哪个才是真正的用户端的真实IP呢？ <br>
     * 答案是取X-Forwarded-For中第一个非unknown的有效IP字符串。<br>
     *
     * 如：X-Forwarded-For：192.168.1.110, 192.168.1.120, 192.168.1.130, <br>
     * 192.168.1.100 <br>
     *
    ...
    public String getToken(ServerHttpRequest request) {
        return request.getHeaders().getFirst(BaseConstant.AUTHORIZATION);
    }
